...
function Get-RelativePath([string]$root, [string]$full) {
  ...
  $filePath = (Resolve-Path -LiteralPath $full).Path
  if (-not $rootPath.EndsWith('\')) { $rootPath += '\' }
  $uriRoot = New-Object System.Uri($rootPath)
  $uriFile = New-Object System.Uri($filePath)
  $relUri = $uriRoot.MakeRelativeUri($uriFile)
